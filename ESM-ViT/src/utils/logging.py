...
def log_results(final_results, args):
    ...
    if args.num_tasks < 8:
        ...
        proxy_n = getattr(args, 'proxy_n_samples', None)
        proxy_suffix = f"_n{proxy_n}" if proxy_n is not None else ""
        save_file = f"results/{args.save_subfolder}/seed{args.seed}_{args.model}_{args.num_tasks}tasks_{args.pca_batch_size}batch_size{proxy_suffix}_{args.method.full_name}_nonlinear_additions_{mask_suffix}.json"
