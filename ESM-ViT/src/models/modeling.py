...
class ImageEncoder(torch.nn.Module):
    def __init__(self, model_name: str, keep_lang=False):
        ...
        if "__pretrained__" in model_name:
            ...
            name = model_name
            pretrained = "openai"
        # OpenCLIP models cache in OPENCLIP_CACHEDIR by default
        if name == 'ViT-B-32':
            name = name + '-quickgelu'
        (
            self.model,
        ...
        # ) = open_clip.create_model_and_transforms(name+'-quickgelu', pretrained=pretrained, cache_dir=OPENCLIP_CACHEDIR)
