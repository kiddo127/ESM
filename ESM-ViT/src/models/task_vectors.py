...
class _TaskVector(abc.ABC):
    ...
    def apply_to(self, pretrained_checkpoint, scaling_coef=1.0, args=None, scaling_keys=None):
        ...
        with torch.no_grad():
            ...
            for key in pretrained_state_dict:
                if key not in self.vector:
                    ...
                    continue
                if scaling_keys is not None and key not in scaling_keys:
                    new_state_dict[key] = (
                        # pretrained_state_dict[key] + scaling_coef * self.vector[key]
                        pretrained_state_dict[key] + self.vector[key].to(args.device) # 自己加的
                    )
                else:
                    new_state_dict[key] = (
                        # pretrained_state_dict[key] + scaling_coef * self.vector[key]
                        pretrained_state_dict[key] + scaling_coef * self.vector[key].to(args.device) # 自己加的
                    )
