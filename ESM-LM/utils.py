...
def save_excel(data, out_path, log_name=None):
    # save excel
    ...
    if log_name:
        xlsx_path = os.path.join(out_path, f'results_{log_name}.csv')
        md_path = os.path.join(out_path, f'results_{log_name}.md')
...
def load_classifier(model_name: str, dtype=torch.float32, save_classifier_head=True):
    model = AutoModelForSequenceClassification.from_pretrained(
        # model_name, torch_dtype=dtype, device_map="cpu", # 本地有了，但是还是显示网络错误，耗时重连
        model_name, torch_dtype=dtype, device_map="cpu", local_files_only=True # 但是要下载时，必须用上面那行
    )
