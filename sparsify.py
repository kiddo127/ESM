...
def svd(
    ...
    **kwargs,
    ...
    # 添加正则化项改善矩阵条件数
    lambda_reg = 1e-9  # 正则化系数，可调整
    I = torch.eye(tensor.size(-2), tensor.size(-1), device=tensor.device, dtype=tensor.dtype)
    regularized_tensor = tensor + lambda_reg * I
    
    driver = None
    if tensor.is_cuda:
        driver = 'gesvda'
    
    U, S, Vh = torch.linalg.svd(regularized_tensor, full_matrices=True, driver=driver)
    new_rank = int(density * len(S))
    U, S, Vh = U[:, :new_rank], S[:new_rank], Vh[:new_rank, :]
    res = U @ torch.diag(S) @ Vh
    return res